...
def load_nwb_list(plot_loc, load_fip = False):
    ...
    bg_files = glob.glob(f"{plot_loc}/**/bg_coef**.csv")
    if bg_files:
        print("loading df_bg")
        dfs = []
        for file in bg_files:
            df = pd.read_csv(file)
            channel = Path(file).stem.removeprefix("bg_coef_")
            df["channel"] = channel
            dfs.append(df)
        df_bg = pd.concat(dfs, ignore_index=True)
        df_bg[["subject_id", "session_id"]] = df_bg["session_id"].str.split("_", n=1, expand=True)

    else:
        print("no bg_coef files found at plot location, skipping")
        df_bg = None

    if slope_files:
        print("loading df_slope")
        df_slope = pd.concat([pd.read_csv(file) for file in slope_files], ignore_index=True)
        df_slope = df_slope.rename(columns = {'date':'session_id', 'channel':'channel_pp'})
    else:
    ...
    return nwbs, df_sess, df_slope, df_bg
